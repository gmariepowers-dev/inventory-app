...
function saveItemChanges() {
  ...
  fetch(`/edit-item/${window.currentItemId}`, {
    ...
    .then(res => res.json())
    .then(() => {
      if (viewManufacturer && modalManufacturer) viewManufacturer.innerText = modalManufacturer.value;
      if (viewDimensions && modalDimensions) viewDimensions.innerText = modalDimensions.value;
      if (viewColors && modalColors) viewColors.innerText = modalColors.value;
      if (viewCost && modalCost) viewCost.innerText = formatCurrency(modalCost.value);
      if (viewRetail && modalRetail) viewRetail.innerText = formatCurrency(modalRetail.value);
      if (viewWeight && modalWeight) viewWeight.innerText = modalWeight.value;
